...
/obj/machinery/power/solar_control/proc/interact(mob/user)
	...
	t += "Sun angle : Approximately [sun.angle]<br><br>"
	...
	t += "SMES Control"
	var/amSM = 0
	for(var/obj/machinery/power/smes/SMESC in SMESes)
		amSM = amSM + 1
		t += "<hr>"
		t += "SMES #[amSM] (distance : [abs(SMESC.x-x)],[abs(SMESC.y-y)])"
		t += "Charging : [SMESC.charging ? "Yes" : "No"]<br>"
		t += "Last output : [SMESC.lastout]<br>"
