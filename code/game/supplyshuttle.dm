...
/proc/supply_ticker()
	dab_coins_currency += SUPPLY_POINTSPER
...
/proc/sell_crates()
	...
	for(var/turf/T in get_area_turfs(shuttleat) )
		...
		if (crate)
			del(crate)
			dab_coins_currency += SUPPLY_POINTSPERCRATE
...
/obj/machinery/computer/ordercomp/Topic(href, href_list)
	...
	if (href_list["order"])
		src.temp = "dabcoins: [dab_coins_currency]<BR><HR><BR>Request what?<BR><BR>"
...
/obj/machinery/computer/supplycomp/Topic(href, href_list)
	...
	if (href_list["order"])
		...
		for(var/S in (typesof(/datum/supply_packs) - /datum/supply_packs) )
			...
			src.temp += "<A href='?src=\ref[src];doorder=[N.type]'>[N.name]</A> Cost: [N.cost] Dab Points<BR>" //the obj because it would get caught by the garbage
		src.temp += "<BR><A href='?src=\ref[src];mainmenu=1'>OK</A>"								//collector. oh well.
		...
		if(locate(href_list["doorder"])) //Comes from the requestlist
			...
			if(dab_coins_currency >= P.cost)
				dab_coins_currency -= P.cost
				O.object = P
				O.orderedby = usr.name
				...
				src.temp = "Not enough dabcoins.<BR>"
				src.temp += "<BR><A href='?src=\ref[src];mainmenu=1'>OK</A>"
	...
	else if (href_list["dabcoins"])
		var/obj/item/weapon/card/C = input(usr,"What card do you want to use?","Dab Coins Addition") in usr.contents
		if(C && istype(C,/obj/item/weapon/card))
			if(C.credit)
				var/cod = input(usr,"Security Code","Dab Coins Addition") as num|null
				if("[cod]" == "[C.credit.code]")
					var/amnt = input(usr,"Amount?","Dab Coins Addition") as num|null
					if(amnt)
						if(amnt > C.credit.dabcoins)
							amnt = C.credit.dabcoins
						C.credit.dabcoins = C.credit.dabcoins - amnt
						dab_coins_currency = dab_coins_currency + amnt
						usr << "<b><font color='green'>Added [amnt] dabcoins to the machine!"
						src.temp = "Added [amnt] dabcoins.<BR>"
						src.temp += "<BR><A href='?src=\ref[src];mainmenu=1'>OK</A>"
				else
					usr << "<b><font color='red'>Wrong code."
