	...
	switch(href_list["operation"])
		...
		if("send_sound")
			...
			if(current_radio_song)
				M << "<font color='red'>Stopped playing sound."
				current_radio_song = null
			else
				current_radio_song = input(M,"What sound would you like to play? Do not abuse this.","Sound Playing") as sound
		if("main")
	...
	switch(src.state)
		if(STATE_DEFAULT)
			if (src.authenticated)
				...
				dat += "<BR>\[ <A HREF='?src=\ref[src];operation=send_sound'>Play Sound Via Radio</A> \]"
