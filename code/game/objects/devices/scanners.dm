...
/obj/item/device/healthanalyzer/attack(mob/M as mob, mob/user as mob)
	...
	if (M.reagents:get_reagent_amount(/datum/reagent/inaprovaline))
		user.show_message(text("\blue Bloodstream Analysis located [M.reagents:get_reagent_amount(/datum/reagent/inaprovaline)] units of rejuvenation chemicals."), 1)
